...
class ObjectParent:
    ...
    def _zh_join(self, names):
        """Join visible names without English conjunctions."""
    ...
    def get_display_exits(self, looker, **kwargs):
        ...
        return f"|w出口：|n {self._zh_join(exit_names)}" if exit_names else ""

    def get_display_characters(self, looker, **kwargs):
        characters = self.filter_visible(
            self.contents_get(content_type="character"), looker, **kwargs
        )
        character_names = [char.get_display_name(looker, **kwargs) for char in characters]
        return f"|w角色：|n {self._zh_join(character_names)}" if character_names else ""

    def get_display_things(self, looker, **kwargs):
        things = self.filter_visible(self.contents_get(content_type="object"), looker, **kwargs)
        thing_names = [thing.get_display_name(looker, **kwargs) for thing in things]
        return f"|w你看到：|n {self._zh_join(sorted(thing_names))}" if thing_names else ""
    ...
    def at_say(
        ...
        **kwargs,
        ...
        if kwargs.get("whisper", False):
            receivers = list(make_iter(receivers)) if receivers else []
            receiver_names = self._zh_join(recv.get_display_name(self) for recv in receivers)
            if msg_self:
                self.msg(text=(f"你對 {receiver_names} 密語：「{message}」", {"type": "whisper"}))
            for receiver in receivers:
                receiver.msg(
                    text=(
                        f"{self.get_display_name(receiver)} 對你密語：「{message}」",
                        {"type": "whisper"},
                    ),
                    from_obj=self,
                )
        ...
        if msg_self:
            self.msg(text=(f"你說：「{message}」", {"type": "say"}), from_obj=self)
        if self.location:
            self.location.msg_contents(
                text=(f"{self.get_display_name()} 說：「{message}」", {"type": "say"}),
                from_obj=self,
                exclude=[self],
            )
